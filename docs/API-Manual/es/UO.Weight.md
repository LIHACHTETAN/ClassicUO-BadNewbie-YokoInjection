...
Lee el contador de status del jugador actual: peso actual en stones.
...
Integer — peso actual en stones, 0..65535 en el modelo. 0 puede ser real, desconocido o Player ausente/destruido. Cantidad, no Boolean, ID ni type: 1 significa una unidad, no éxito. No enumera objetos ni devuelve un array.

## Comportamiento

- No admite argumentos. Usar las firmas mostradas.
- Lee Player.Weight en el hilo del juego si Player existe y no está destruido; de lo contrario 0. Sin búsqueda de equipo, cálculo de bonos, petición status ni espera. Un fantasma presente no es un Player destruido.
- Weight lee el total en stones enviado por el servidor. El shard decide qué pesos del cuerpo, equipo y bolsas anidadas incluye. No recorre contenedores, suma pesos ni actualiza bolsas cerradas. No es Weight de un objeto ni cantidad de items.
...
# Lee el contador de status del jugador actual: peso actual en stones.
#
# Integer — peso actual en stones, 0..65535 en el modelo. 0 puede ser real, desconocido o Player
