...
Lee el valor previsto de la altura Z tras los pasos del jugador ya en cola.
...
Integer Z con signo -128..127; conserva alturas negativas. 0 puede ser válido o jugador ausente.
...
- X/Y son coordenadas del mundo/mapa, no píxeles del gump de un contenedor. Z es altura, no planta. Devuelve un componente, no un array ni el destino final de la ruta.
...
# Lee el valor previsto de la altura Z tras los pasos del jugador ya en cola.
#
# Integer Z con signo -128..127; conserva alturas negativas. 0 puede ser válido o jugador
# ausente.

SUB Main()
    # predicted guarda una llamada sin argumentos; CStr formatea el número para el diario.

    VAR predicted = UO.PredictedZ()
...
SUB Main()
    ...
    VAR before = UO.PredictedZ()
    WAIT(100)
    VAR after = UO.PredictedZ()
