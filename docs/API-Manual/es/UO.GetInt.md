...
Lee la Inteligencia actual (INT).
...
- `ObjID` — ObjID solo en la forma de un argumento: serial entero, cadena decimal/hexadecimal 0x, "self", "lasttarget" o nombre AddObject. Identifica al personaje, no graphic/type. Al omitirlo, lee al jugador actual.
...
- Formas admitidas: () para self y (ObjID) para un sujeto explícito. Sin otros argumentos opcionales.
- Lee Player.Intelligence si Player existe y no está destruido; en caso contrario, 0. Un personaje muerto presente no es un objeto destruido. No se deduce de HP, maná o resistencia.
...
- Nombres equivalentes con y sin UO., sin distinguir mayúsculas: `Int Intelligence GetInt GetIntelligence`.
...
- ExecuteStealthCompatibility.Arg resuelve el nombre antes de convertir el número. Un texto desconocido no numérico causa un error, no un cursor. Decimal se convierte a Integer; Array/Unit a 0. Usar un serial válido.
...
ObjID solo en la forma de un argumento: serial entero, cadena decimal/hexadecimal 0x, "self", "lasttarget" o nombre AddObject. Identifica al personaje, no graphic/type. Al omitirlo, lee al jugador actual.
...
CharacterStatus asigna el campo STR/DEX/INT recibido a Player.Intelligence al procesar un paquete status propio aplicable. La consulta lee la caché sin esperar otro paquete.
...
### Seleccionar por serial y nombre
...
SUB Main()
    # id procede de UO.Self(), no de type. AddObject guarda statSubject sin cursor porque se
    # proporciona el segundo argumento. Ambas llamadas seleccionan el mismo sujeto; las lecturas no
    # son atómicas.
...
- id procede de UO.Self(), no de type. AddObject guarda statSubject sin cursor porque se proporciona el segundo argumento. Ambas llamadas seleccionan el mismo sujeto; las lecturas no son atómicas.
