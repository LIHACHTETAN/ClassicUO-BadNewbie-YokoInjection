...
Devuelve el ID del personaje actual.
...
Integer — serial/ID, no graphic/type, capa, cantidad ni Boolean. 0: no hay objeto actual. Conserva los 32 bits; comprueba <> 0, no = TRUE o > 0. Un resultado guardado no se actualiza solo. Lee World.Player.Serial; Player ausente/destruido da 0. Morir no es destruirse: un fantasma presente conserva su ID.
...
Lee World.Player.Serial; Player ausente/destruido da 0. Morir no es destruirse: un fantasma presente conserva su ID. Invoke lee en el hilo del juego; un hilo de trabajo espera el procesamiento del gestor. Cancelar el script interrumpe esa espera. No añade demoras ni consultas de red.
...
# solo. Lee World.Player.Serial; Player ausente/destruido da 0. Morir no es destruirse: un
# fantasma presente conserva su ID.
