...
- `radians` — 필수: Integer/Decimal 또는 소수점과 선택적 지수가 있는 십진 텍스트(예: "-1.25e2"). UI 언어와 무관합니다. 잘못된/16진 텍스트, Unit, Array, Object는 0입니다. 숫자 16진 리터럴은 이미 Integer입니다. NaN/Infinity도 가능합니다. 라디안 입력입니다. 결과는 제한이 없고 pi/2+k*pi 근처에서 크고 불안정합니다. NaN/무한대는 NaN입니다.

## 반환값

Decimal — tan(radians). 라디안 입력입니다. 결과는 제한이 없고 pi/2+k*pi 근처에서 크고 불안정합니다. NaN/무한대는 NaN입니다. 숫자이며 ID나 성공 플래그가 아닙니다. 1/0을 성공/실패로 해석하지 마세요.
...
- 라디안 입력입니다. 결과는 제한이 없고 pi/2+k*pi 근처에서 크고 불안정합니다. NaN/무한대는 NaN입니다.
...
# 탄젠트를 계산합니다.
#
# Decimal — tan(radians). 라디안 입력입니다. 결과는 제한이 없고 pi/2+k*pi 근처에서 크고 불안정합니다. NaN/무한대는 NaN입니다. 숫자이며
...
SUB Main()
    # radians = 0.7853981633974483; 예상 결과 ~1(~는 근사). value가 결과를 저장하며 CStr은 Print용으로 형식화합니다.
...
SUB Main()
    # degrees는 도이며 pi/180으로 라디안 변환 후 Tan을 호출합니다. TangentDegrees(45)≈1; 특이각을 피합니다.
