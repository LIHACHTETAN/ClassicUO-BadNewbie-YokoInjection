...
직사각형 안의 지형 타일을 graphic/type으로 검색합니다.
...
- `TileType` — 타일의 graphic/type 하나이며 보통 0..65535입니다. serial이 아닙니다. 0은 graphic 0만 찾으며 -1은 모든 타입을 뜻하지 않습니다. 일치하지 않으면 빈 결과입니다.

## 반환값

각 레코드가 [graphic, X, Y, Z]인 Array이며 모든 필드는 Integer입니다. graphic은 지형 타입, Z는 바닥 높이입니다. 일치 항목이 없으면 빈 Array입니다. 개수는 GetArrayLength(result)로 읽습니다. 인덱스는 0부터 시작합니다. Boolean, serial, Pascal record가 아니며 일곱 번째 출력 인수도 없습니다.
...
# 직사각형 안의 지형 타일을 graphic/type으로 검색합니다.
#
# 각 레코드가 [graphic, X, Y, Z]인 Array이며 모든 필드는 Integer입니다. graphic은 지형 타입, Z는 바닥 높이입니다. 일치 항목이 없으면
# 빈 Array입니다. 개수는 GetArrayLength(result)로 읽습니다. 인덱스는 0부터 시작합니다. Boolean, serial, Pascal record가
# 아니며 일곱 번째 출력 인수도 없습니다.
