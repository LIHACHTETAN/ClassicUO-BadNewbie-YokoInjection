...
현재 플레이어의 상태 계수를 읽습니다: 최대 무게(stones).
...
Integer — 최대 무게(stones), 모델 범위 0..65535입니다. 0은 실제 값, 미수신 데이터 또는 없거나 파괴된 Player일 수 있습니다. 수량이며 Boolean, ID, type이 아닙니다. 1은 한 단위이고 성공이 아닙니다. 객체를 열거하거나 배열을 반환하지 않습니다.

## 동작

- 인자가 없습니다. 표시된 정확한 시그니처를 사용하세요.
- 게임 스레드에서 Player.WeightMax를 읽습니다. 현재 Player가 없거나 파괴되었으면 0입니다. 장비 검색, 보너스 계산, 상태 요청, 갱신 대기가 없습니다. 존재하는 유령은 파괴된 Player와 다릅니다.
- MaxWeight는 캐시 WeightMax를 읽습니다. 자신의 status type >= 5는 서버 UInt16 상한을 사용하며 0도 유지합니다. 구형 확장 상태는 수신 시 한 번 계산합니다: UO 프로토콜 >= 5.0.0a는 7 * floor(STR / 2) + 40, 더 오래된 것은 STR * 4 + 25입니다. UInt16에 저장하므로 극단값은 65536 모듈로 순환합니다. STR=101은 각각 390 또는 429입니다. STR이 따로 변경되어도 getter는 재계산하지 않습니다.
...
# 현재 플레이어의 상태 계수를 읽습니다: 최대 무게(stones).
#
# Integer — 최대 무게(stones), 모델 범위 0..65535입니다. 0은 실제 값, 미수신 데이터 또는 없거나 파괴된 Player일 수 있습니다. 수량이며
