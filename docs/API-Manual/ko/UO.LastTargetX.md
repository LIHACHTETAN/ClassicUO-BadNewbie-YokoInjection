...
Integer — 저장된 X 좌표로 픽셀이나 Boolean이 아닙니다. 선택 전/Clear 후 또는 알 수 없는 객체는 0이며 실제 좌표 0도 유효합니다. LastTarget()=0인 지면/정적 대상도 지원합니다.
...
프로젝트 소스: `external/InjectionScript/src/InjectionScript/Runtime/InjectionApiUO.cs`; 함수 `LastTargetX`.
...
# 마지막 대상 선택 때 저장한 X를 읽습니다.
#
# Integer — 저장된 X 좌표로 픽셀이나 Boolean이 아닙니다. 선택 전/Clear 후 또는 알 수 없는 객체는 0이며 실제 좌표 0도 유효합니다.
