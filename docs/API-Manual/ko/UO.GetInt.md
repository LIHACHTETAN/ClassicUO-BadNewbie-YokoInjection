...
- `ObjID` — ObjID는 표시된 한 인자 형식만 지원합니다. 캐릭터 serial 정수, 십진/0x 16진 문자열, "self", "lasttarget", AddObject 이름을 받습니다. graphic/type이 아닌 대상을 선택하며 생략하면 현재 플레이어입니다.
...
- ()는 self, (ObjID)는 명시한 대상입니다. 다른 선택 인자는 없습니다.
- Player가 존재하고 파괴되지 않았을 때 Player.Intelligence를 읽으며, 아니면 0입니다. 존재하는 죽은 캐릭터는 파괴된 객체가 아닙니다. HP, 마나, 스태미나에서 계산하지 않습니다.
...
- UO. 유무와 대소문자에 관계없이 같은 능력치를 읽는 이름: `Int Intelligence GetInt GetIntelligence`.
- UO. 없는 Int(value)는 BASIC 숫자를 내림하고 Str(value)는 텍스트로 서식화합니다. 능력치를 읽는 UO.Int()/UO.Str()와 다른 연산입니다. GetInt(ObjID)는 숫자 반올림이 아닙니다.
- ExecuteStealthCompatibility.Arg는 수치 변환 전에 객체 이름을 해석합니다. 알 수 없는 비수치 문자열은 변환 오류이며 타깃을 열지 않습니다. Decimal은 Integer로, Array/Unit은 0으로 변환됩니다. 유효한 serial을 사용하세요.
...
ObjID는 표시된 한 인자 형식만 지원합니다. 캐릭터 serial 정수, 십진/0x 16진 문자열, "self", "lasttarget", AddObject 이름을 받습니다. graphic/type이 아닌 대상을 선택하며 생략하면 현재 플레이어입니다.
...
프로젝트 소스: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 함수 `GetIntelligence`.

#### 4. Invoke

Player가 존재하고 파괴되지 않았을 때 Player.Intelligence를 읽으며, 아니면 0입니다. 존재하는 죽은 캐릭터는 파괴된 객체가 아닙니다. HP, 마나, 스태미나에서 계산하지 않습니다. Invoke는 게임 스레드에서 읽고 대기는 스크립트 취소에 대응합니다. status 요청, 타깃, 패킷 전송, 능력치 변경, 내장 지연이 없습니다.
...
CharacterStatus는 해당하는 자신 status 패킷의 STR/DEX/INT 필드를 Player.Intelligence에 대입합니다. 조회는 이 캐시를 읽고 새 패킷을 기다리지 않습니다.
...
SUB Main()
    # id는 type이 아닌 UO.Self()의 serial입니다. AddObject에 두 번째 인자가 있어 타깃 없이 statSubject로 저장합니다. 두 호출은 같은
    # 대상을 선택하지만 읽기는 원자적이지 않습니다.
...
- id는 type이 아닌 UO.Self()의 serial입니다. AddObject에 두 번째 인자가 있어 타깃 없이 statSubject로 저장합니다. 두 호출은 같은 대상을 선택하지만 읽기는 원자적이지 않습니다.
