...
Integer — 행운 점수, 모델 범위 0..65535입니다. 0은 실제 값, 미수신 데이터 또는 없거나 파괴된 Player일 수 있습니다. 수량이며 Boolean, ID, type이 아닙니다. 1은 한 단위이고 성공이 아닙니다. 객체를 열거하거나 배열을 반환하지 않습니다.

## 동작

- 인자가 없습니다. 표시된 정확한 시그니처를 사용하세요.
- 게임 스레드에서 Player.Luck를 읽습니다. 현재 Player가 없거나 파괴되었으면 0입니다. 장비 검색, 보너스 계산, 상태 요청, 갱신 대기가 없습니다. 존재하는 유령은 파괴된 Player와 다릅니다.
- Luck는 status type >= 4의 서버 행운 점수를 UInt16으로 읽습니다. 백분율, 난수, 전리품 품질 보장이 아닙니다. 읽을 때 드롭 확률을 계산하거나 장비 보너스를 더하지 않습니다.
...
# 현재 플레이어의 상태 계수를 읽습니다: 행운 점수.
#
# Integer — 행운 점수, 모델 범위 0..65535입니다. 0은 실제 값, 미수신 데이터 또는 없거나 파괴된 Player일 수 있습니다. 수량이며 Boolean,
# ID, type이 아닙니다. 1은 한 단위이고 성공이 아닙니다. 객체를 열거하거나 배열을 반환하지 않습니다.
...
SUB Main()
    # LuckAtLeast(minimum)은 숫자 조건을 받으며 1000은 예시이고 상한이나 백분율이 아닙니다. 없는 Player를 거부하고 행운을 한 번 읽어 value
    # >= minimum에 대해 Integer Boolean 1=TRUE 또는 0=FALSE를 반환합니다. 행운 자체는 Boolean이 아닙니다. 캐시 비교이며 드롭을
    # 예측하지 않습니다.
...
- LuckAtLeast(minimum)은 숫자 조건을 받으며 1000은 예시이고 상한이나 백분율이 아닙니다. 없는 Player를 거부하고 행운을 한 번 읽어 value >= minimum에 대해 Integer Boolean 1=TRUE 또는 0=FALSE를 반환합니다. 행운 자체는 Boolean이 아닙니다. 캐시 비교이며 드롭을 예측하지 않습니다.
