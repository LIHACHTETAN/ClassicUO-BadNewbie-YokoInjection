...
클라이언트가 마지막으로 상태를 수락한 객체의 serial을 반환합니다.
...
Integer — serial의 32비트이며 type이나 Boolean이 아닙니다. 0은 아직 수락한 상태가 없거나 세계를 초기화했음을 뜻합니다. 최상위 비트를 보존하므로 음수도 serial을 나타낼 수 있습니다. > 0 또는 = TRUE 대신 <> 0을 확인하세요.
...
# 클라이언트가 마지막으로 상태를 수락한 객체의 serial을 반환합니다.
#
# Integer — serial의 32비트이며 type이나 Boolean이 아닙니다. 0은 아직 수락한 상태가 없거나 세계를 초기화했음을 뜻합니다. 최상위 비트를
# 보존하므로 음수도 serial을 나타낼 수 있습니다. > 0 또는 = TRUE 대신 <> 0을 확인하세요.
