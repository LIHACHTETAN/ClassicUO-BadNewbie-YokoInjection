...
- 정리 후 Try를 벗어난 오류가 외부 On Error GoTo 처리기로 가면 Resume은 헤더부터 전체 Try를 재시도합니다. Resume Next와 On Error Resume Next는 End Try 바로 다음 문장에서 계속합니다. 완료한 작업이 반복될 수 있으며 정리된 본문 중간으로 돌아가지 않습니다.
