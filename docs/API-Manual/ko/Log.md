...
- `number` — 필수: Integer/Decimal 또는 소수점과 선택적 지수가 있는 십진 텍스트(예: "-1.25e2"). UI 언어와 무관합니다. 잘못된/16진 텍스트, Unit, Array, Object는 0입니다. 숫자 16진 리터럴은 이미 Integer입니다. NaN/Infinity도 가능합니다. 밑은 e이며 10이 아닙니다. Log(1)=0, Log(0)=-Infinity, 음수는 NaN, +Infinity는 Infinity입니다.

## 반환값

Decimal — ln(number). 밑은 e이며 10이 아닙니다. Log(1)=0, Log(0)=-Infinity, 음수는 NaN, +Infinity는 Infinity입니다. 숫자이며 ID나 성공 플래그가 아닙니다. 1/0을 성공/실패로 해석하지 마세요.
...
- 밑은 e이며 10이 아닙니다. Log(1)=0, Log(0)=-Infinity, 음수는 NaN, +Infinity는 Infinity입니다.
...
# 자연로그를 계산합니다.
#
# Decimal — ln(number). 밑은 e이며 10이 아닙니다. Log(1)=0, Log(0)=-Infinity, 음수는 NaN, +Infinity는
# Infinity입니다. 숫자이며 ID나 성공 플래그가 아닙니다. 1/0을 성공/실패로 해석하지 마세요.

SUB Main()
    # number = 1; 예상 결과 0(~는 근사). value가 결과를 저장하며 CStr은 Print용으로 형식화합니다.
...
SUB Main()
    # number = 2.718281828459045; 예상 결과 ~1(~는 근사). value가 결과를 저장하며 CStr은 Print용으로 형식화합니다.
...
SUB Main()
    # value>0, baseValue>0 및 <>1입니다. Log(value)/Log(baseValue)로 LogBase(100,10)≈2. 잘못된 입력의 대체값 0은
    # 유효한 로그값일 수도 있습니다.
...
- value>0, baseValue>0 및 <>1입니다. Log(value)/Log(baseValue)로 LogBase(100,10)≈2. 잘못된 입력의 대체값 0은 유효한 로그값일 수도 있습니다.
