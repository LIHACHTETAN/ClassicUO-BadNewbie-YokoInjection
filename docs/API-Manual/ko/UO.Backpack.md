...
Integer — serial/ID이며 graphic/type, 레이어, 수량, Boolean이 아닙니다. 0은 현재 객체 없음입니다. 32비트를 모두 보존하므로 = TRUE 또는 > 0 대신 <> 0으로 검사합니다. 저장한 결과는 자동 갱신되지 않습니다. Player의 Backpack 레이어에서 파괴되지 않은 아이템을 찾습니다. Player나 배낭이 없거나 파괴되었으면 0입니다. 내용물이 아닌 컨테이너 ID이며 새 가방을 만들지 않습니다.
...
Player의 Backpack 레이어에서 파괴되지 않은 아이템을 찾습니다. Player나 배낭이 없거나 파괴되었으면 0입니다. 내용물이 아닌 컨테이너 ID이며 새 가방을 만들지 않습니다. Invoke는 게임 스레드에서 읽습니다. 작업 스레드는 관리자의 처리를 기다리며 스크립트 취소가 대기를 중단합니다. 추가 지연이나 네트워크 요청은 없습니다.
...
Player의 Backpack 레이어에서 파괴되지 않은 아이템을 찾습니다. Player나 배낭이 없거나 파괴되었으면 0입니다. 내용물이 아닌 컨테이너 ID이며 새 가방을 만들지 않습니다.

Integer — serial/ID이며 graphic/type, 레이어, 수량, Boolean이 아닙니다. 0은 현재 객체 없음입니다. 32비트를 모두 보존하므로 = TRUE 또는 > 0 대신 <> 0으로 검사합니다. 저장한 결과는 자동 갱신되지 않습니다.

프로젝트 소스: `src/ClassicUO.Client/Game/GameObjects/Entity.cs`; 함수 `FindItemByLayer`.
...
# TRUE 또는 > 0 대신 <> 0으로 검사합니다. 저장한 결과는 자동 갱신되지 않습니다. Player의 Backpack 레이어에서 파괴되지 않은 아이템을 찾습니다.
# Player나 배낭이 없거나 파괴되었으면 0입니다. 내용물이 아닌 컨테이너 ID이며 새 가방을 만들지 않습니다.
