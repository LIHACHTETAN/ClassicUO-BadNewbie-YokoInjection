...
현재 플레이어의 상태 계수를 읽습니다: 사용 중인 추종자 제어 슬롯.
...
Integer — 사용 중인 추종자 제어 슬롯, 모델 범위 0..255입니다. 0은 실제 값, 미수신 데이터 또는 없거나 파괴된 Player일 수 있습니다. 수량이며 Boolean, ID, type이 아닙니다. 1은 한 단위이고 성공이 아닙니다. 객체를 열거하거나 배열을 반환하지 않습니다.

## 동작

- 인자가 없습니다. 표시된 정확한 시그니처를 사용하세요.
- 게임 스레드에서 Player.Followers를 읽습니다. 현재 Player가 없거나 파괴되었으면 0입니다. 장비 검색, 보너스 계산, 상태 요청, 갱신 대기가 없습니다. 존재하는 유령은 파괴된 Player와 다릅니다.
- PetsCurrent/Followers는 자신의 status type >= 3에서 사용 중인 제어 슬롯을 읽습니다. 한 마리가 여러 슬롯을 쓸 수 있으며 소환/길들인 생물의 규칙은 서버가 정합니다. 보이는 mobile 수, 펫 ID 목록, 근처 동물 수가 아닙니다.
...
# 현재 플레이어의 상태 계수를 읽습니다: 사용 중인 추종자 제어 슬롯.
#
# Integer — 사용 중인 추종자 제어 슬롯, 모델 범위 0..255입니다. 0은 실제 값, 미수신 데이터 또는 없거나 파괴된 Player일 수 있습니다. 수량이며
