...
현재 플레이어의 대기열에 들어 있는 이동 단계 이후 예상되는 방향 값을 읽습니다.
...
Integer 방향 0..7, 달리기 비트를 제거합니다. 0은 북쪽이거나 플레이어가 없다는 뜻입니다.
...
- 월드 방향: 0 북, 1 북동, 2 동, 3 남동, 4 남, 5 남서, 6 서, 7 북서. 화면은 등각 투영입니다. Direction.Mask는 달리기 플래그를 제거하며 속도를 반환하지 않습니다.
