...
Legge il contatore status del giocatore attuale: punti fortuna.
...
Integer — punti fortuna, 0..65535 nel modello. 0 può essere reale, sconosciuto o Player assente/distrutto. Quantità, non Boolean, ID o type: 1 significa un’unità, non successo. Nessuna ricerca oggetti o array restituito.

## Comportamento

- Nessun argomento. Seguire le firme visualizzate.
- Legge Player.Luck nel thread di gioco se Player esiste e non è distrutto; altrimenti 0. Nessuna ricerca nell’equipaggiamento, calcolo bonus, richiesta status o attesa. Un fantasma presente non è un Player distrutto.
- Luck legge punti fortuna del server, UInt16, da status type >= 4. Non è percentuale, numero casuale o garanzia di bottino. La lettura non calcola probabilità né aggiunge bonus dell’equipaggiamento.
...
# Legge il contatore status del giocatore attuale: punti fortuna.
#
# Integer — punti fortuna, 0..65535 nel modello. 0 può essere reale, sconosciuto o Player
# assente/distrutto. Quantità, non Boolean, ID o type: 1 significa un’unità, non successo.
# Nessuna ricerca oggetti o array restituito.
...
SUB Main()
    # LuckAtLeast(minimum) riceve un requisito numerico; 1000 è un esempio, non limite o
    # percentuale. Rifiuta Player assente, legge una volta e restituisce Integer Boolean 1=TRUE o
    # 0=FALSE per value >= minimum. La fortuna non è Boolean. Confronto della cache, nessuna
    # previsione del bottino.
...
- LuckAtLeast(minimum) riceve un requisito numerico; 1000 è un esempio, non limite o percentuale. Rifiuta Player assente, legge una volta e restituisce Integer Boolean 1=TRUE o 0=FALSE per value >= minimum. La fortuna non è Boolean. Confronto della cache, nessuna previsione del bottino.
