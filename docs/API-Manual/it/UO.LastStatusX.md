...
Restituisce la coordinata X salvata dell’ultimo oggetto con stato accettato.
...
Integer — coordinata X salvata, non pixel della finestra né Boolean. 0 prima del primo stato o dopo l’azzeramento; zero è anche una coordinata valida. Usare UO.LastStatus() per distinguere un record assente.
...
Sorgente del progetto: `external/InjectionScript/src/InjectionScript/Runtime/InjectionApiUO.cs`; funzione `LastStatusX`.
...
# Restituisce la coordinata X salvata dell’ultimo oggetto con stato accettato.
#
# Integer — coordinata X salvata, non pixel della finestra né Boolean. 0 prima del primo stato o
