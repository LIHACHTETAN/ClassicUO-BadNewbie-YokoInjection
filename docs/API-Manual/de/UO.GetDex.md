...
Liest die aktuelle Geschicklichkeit (Dexterity, DEX).
...
- `ObjID` — ObjID nur in der Form mit einem Argument: Integer-Serial, dezimaler/0x-Hex-Text, "self", "lasttarget" oder AddObject-Name. Bezeichnet den Charakter, nicht Graphic/Type. Ohne Argument wird der aktuelle Spieler gelesen.
...
- Erlaubt sind () für self und (ObjID) für ein bestimmtes Subjekt. Keine weiteren optionalen Argumente.
- Liest Player.Dexterity, wenn Player vorhanden und nicht zerstört ist, sonst 0. Ein vorhandener toter Charakter ist kein zerstörtes Objekt. Keine Ableitung aus HP, Mana oder Ausdauer.
...
- Gleichwertige Namen mit oder ohne UO., unabhängig von Groß-/Kleinschreibung: `Dex Dexterity GetDex GetDexterity`.
...
- ExecuteStealthCompatibility.Arg löst zuerst Objektnamen auf. Unbekannter nichtnumerischer Text verursacht einen Konvertierungsfehler, keinen Zielcursor. Decimal wird zu Integer, Array/Unit zu 0. Eine gültige Serial verwenden.
...
ObjID nur in der Form mit einem Argument: Integer-Serial, dezimaler/0x-Hex-Text, "self", "lasttarget" oder AddObject-Name. Bezeichnet den Charakter, nicht Graphic/Type. Ohne Argument wird der aktuelle Spieler gelesen.
...
Projektquelle: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; Funktion `GetDexterity`.

#### 4. Invoke

Liest Player.Dexterity, wenn Player vorhanden und nicht zerstört ist, sonst 0. Ein vorhandener toter Charakter ist kein zerstörtes Objekt. Keine Ableitung aus HP, Mana oder Ausdauer. Invoke liest auf dem Spielthread; Warten kann durch Skriptabbruch enden. Kein Paket, Statusabruf, Zielcursor, Ändern des Attributs oder eingebauter Wartezeitraum.
...
CharacterStatus schreibt das empfangene STR/DEX/INT-Feld nach Player.Dexterity, wenn ein passendes Statuspaket für den eigenen Spieler eintrifft. Die Abfrage liest den Cache ohne auf ein neues Paket zu warten.
...
### Spieler über Serial und Namen wählen
...
SUB Main()
    # id kommt aus UO.Self(), kein Type. AddObject speichert statSubject ohne Zielcursor, weil das
    # zweite Argument vorhanden ist. Beide Aufrufe wählen denselben Spieler; getrennte Lesevorgänge
    # sind nicht atomar.
...
- id kommt aus UO.Self(), kein Type. AddObject speichert statSubject ohne Zielcursor, weil das zweite Argument vorhanden ist. Beide Aufrufe wählen denselben Spieler; getrennte Lesevorgänge sind nicht atomar.
