...
Liefert die ID des aktuell angelegten Rucksacks.
...
Integer — Serial/ID, kein Graphic/Type, Layer, Zähler oder Boolean. 0: kein aktuelles Objekt. Alle 32 Bits bleiben erhalten; <> 0 statt = TRUE oder > 0 prüfen. Ein gespeichertes Ergebnis aktualisiert sich nicht selbst. Sucht das nicht zerstörte Item auf dem Backpack-Layer des Player. Fehlender/zerstörter Player oder Rucksack liefert 0. Bezeichnet den Container, nicht seinen Inhalt; erstellt keine Tasche.
...
Sucht das nicht zerstörte Item auf dem Backpack-Layer des Player. Fehlender/zerstörter Player oder Rucksack liefert 0. Bezeichnet den Container, nicht seinen Inhalt; erstellt keine Tasche. Invoke liest im Spielthread; ein Arbeitsthread wartet auf die Verarbeitung durch den Manager. Skriptabbruch unterbricht das Warten. Keine zusätzliche Verzögerung oder Netzwerkanfrage.
...
Sucht das nicht zerstörte Item auf dem Backpack-Layer des Player. Fehlender/zerstörter Player oder Rucksack liefert 0. Bezeichnet den Container, nicht seinen Inhalt; erstellt keine Tasche.

Integer — Serial/ID, kein Graphic/Type, Layer, Zähler oder Boolean. 0: kein aktuelles Objekt. Alle 32 Bits bleiben erhalten; <> 0 statt = TRUE oder > 0 prüfen. Ein gespeichertes Ergebnis aktualisiert sich nicht selbst.

Projektquelle: `src/ClassicUO.Client/Game/GameObjects/Entity.cs`; Funktion `FindItemByLayer`.
...
# aktualisiert sich nicht selbst. Sucht das nicht zerstörte Item auf dem Backpack-Layer des
# Player. Fehlender/zerstörter Player oder Rucksack liefert 0. Bezeichnet den Container, nicht
# seinen Inhalt; erstellt keine Tasche.
