...
Reads the player’s current Intelligence (INT).
...
- Reads Player.Intelligence when Player exists and is not destroyed; otherwise 0. A present dead character is not a destroyed object. The client does not derive this attribute from HP, mana or stamina.
...
- Equivalent attribute names with and without UO., ignoring letter case: `Int Intelligence GetInt GetIntelligence`.
...
Project source: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; function `GetIntelligence`.

#### 4. Invoke

Reads Player.Intelligence when Player exists and is not destroyed; otherwise 0. A present dead character is not a destroyed object. The client does not derive this attribute from HP, mana or stamina. Invoke reads on the game thread and honours script cancellation while waiting. No status request, target, network packet, attribute change or built-in delay is issued.
...
CharacterStatus assigns the received STR/DEX/INT field to Player.Intelligence when an applicable own-character status packet is handled. These queries only read that cache; they do not wait for a new packet.
