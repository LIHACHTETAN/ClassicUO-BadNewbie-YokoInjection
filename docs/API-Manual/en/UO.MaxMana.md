...
Reads maximum mana from the local model.
...
Integer — field ManaMax, not a percentage or Boolean. Zero can be a real value or missing data. Never divide by a zero maximum. Other mobiles’ values may be unavailable. HP/HitsMax may be a relative server scale instead of exact points. HP=0 does not prove death; use Dead/IsDead for that state.
...
# Reads maximum mana from the local model.
#
# Integer — field ManaMax, not a percentage or Boolean. Zero can be a real value or missing
