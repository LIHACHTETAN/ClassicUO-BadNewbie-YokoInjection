...
Reads the skill cap received from the server.
...
Decimal (Double) — skill points in tenths, for example 95.1 rather than 951. Field CapFixed is divided by 10 directly as Double. Zero means a zero skill or missing player/skill. This is not Boolean. Legacy SkillVal/BaseVal use a different scale; do not mix the families.
...
# Reads the skill cap received from the server.
#
# Decimal (Double) — skill points in tenths, for example 95.1 rather than 951. Field CapFixed is
# divided by 10 directly as Double. Zero means a zero skill or missing player/skill. This is not
# Boolean. Legacy SkillVal/BaseVal use a different scale; do not mix the families.
