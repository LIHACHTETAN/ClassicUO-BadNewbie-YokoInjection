...
- `RadiobuttonID` — Integer — exact ID of the corresponding control type. Not an index, GumpID or window serial. 0 is an ordinary ID and never means the first control.
- `Value` — Integer — 1 enables, 0 disables. Other nonzero numbers also enable; prefer 0/1. Selecting a radio button deselects its siblings in the same group.
