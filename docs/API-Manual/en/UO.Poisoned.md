...
Checks whether a mobile is poisoned.
...
Integer Boolean: 1 = TRUE, 0 = FALSE. Compare with numbers or unquoted logical constants. 1 means the mobile has IsPoisoned; otherwise, or without a mobile, 0. Poison strength and remaining time are not returned.
...
- Poisoned/Flying follow the protocol version: before 7.0.0.0 bit 0x04 means poison; from 7.0.0.0 it means flight and poison is stored separately. Flight is not derived from Z.
...
1 means the mobile has IsPoisoned; otherwise, or without a mobile, 0. Poison strength and remaining time are not returned.
...
# means the mobile has IsPoisoned; otherwise, or without a mobile, 0. Poison strength and
# remaining time are not returned.
#
...
SUB Main()
    ...
    VAR before = UO.Poisoned()
    WAIT(500)
    VAR after = UO.Poisoned()
