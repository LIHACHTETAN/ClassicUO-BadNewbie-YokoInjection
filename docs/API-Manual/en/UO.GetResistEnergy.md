...
Reads the current player’s resistance field: energy.
...
- The game-thread read takes Player.EnergyResistance if the current Player exists and is not destroyed; otherwise 0. It does not search equipment, recompute bonuses, request status or wait for an update. A present ghost is not the same as a destroyed Player.
