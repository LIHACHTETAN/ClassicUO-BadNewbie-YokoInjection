...
Reads the current player’s status counter: maximum follower control slots.
...
Integer — maximum follower control slots, 0..255 in the client model. 0 can be a real value, unavailable data or a missing/destroyed Player. This is a quantity, not Boolean, ID or type: 1 means one unit, not success. It does not enumerate objects or return an array.

## Behavior

- This command takes no arguments. Use the exact signatures shown above.
- The game-thread read takes Player.FollowersMax if the current Player exists and is not destroyed; otherwise 0. It does not search equipment, recompute bonuses, request status or wait for an update. A present ghost is not the same as a destroyed Player.
- PetsMax/FollowersMax reports the server’s total allowed control slots from own status type >= 3, not the free slots. Current usage can exceed this limit. To display free capacity, compute maximum - current and clamp negative results to 0. This query does not release, tame or summon anything.
...
# Reads the current player’s status counter: maximum follower control slots.
#
# Integer — maximum follower control slots, 0..255 in the client model. 0 can be a real value,
...
SUB Main()
    # CanAddFollower(extraSlots) receives required control slots; 2 is an example for one creature
    # requiring two slots, not necessarily two creatures. It rejects negative input, missing Player
    # or maximum <= 0, reads occupied/maximum and returns Integer Boolean 1=TRUE or 0=FALSE for
    # extraSlots <= maximum - current. No addition overflow; over-capacity fails even for zero. It
    # does not check ownership, taming skill or server approval. Separate reads may see different
    # updates.
...
- CanAddFollower(extraSlots) receives required control slots; 2 is an example for one creature requiring two slots, not necessarily two creatures. It rejects negative input, missing Player or maximum <= 0, reads occupied/maximum and returns Integer Boolean 1=TRUE or 0=FALSE for extraSlots <= maximum - current. No addition overflow; over-capacity fails even for zero. It does not check ownership, taming skill or server approval. Separate reads may see different updates.
