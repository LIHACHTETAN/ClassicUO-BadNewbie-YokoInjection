...
Finds Static/Multi/Item records by graphic/type in a rectangle.
...
- `TileType` — One tile graphic/type, normally 0..65535, not a serial. 0 matches graphic 0 exactly; -1 is not a wildcard. An unmatched type returns no records.

## Returns

Array of [graphic, X, Y, Z, hue] records, all Integer. Z is base height; hue is color. Multiple records at one cell remain separate. No matches: empty Array. Count records with GetArrayLength(result). Indexes start at 0. The result is not a Boolean, serial, or Pascal record; there is no seventh output parameter.
...
- The current static branch includes Static, Multi and Item records, including loaded ground items, but excludes Land and Mobile. Its scope is wider than file-only statics.
...
# Finds Static/Multi/Item records by graphic/type in a rectangle.
#
# Array of [graphic, X, Y, Z, hue] records, all Integer. Z is base height; hue is color.
# Multiple records at one cell remain separate. No matches: empty Array. Count records with
# GetArrayLength(result). Indexes start at 0. The result is not a Boolean, serial, or Pascal
# record; there is no seventh output parameter.
