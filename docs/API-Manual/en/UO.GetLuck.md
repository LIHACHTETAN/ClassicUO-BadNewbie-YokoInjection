...
Reads the current player’s status counter: luck points.
...
Integer — luck points, 0..65535 in the client model. 0 can be a real value, unavailable data or a missing/destroyed Player. This is a quantity, not Boolean, ID or type: 1 means one unit, not success. It does not enumerate objects or return an array.

## Behavior

- This command takes no arguments. Use the exact signatures shown above.
- The game-thread read takes Player.Luck if the current Player exists and is not destroyed; otherwise 0. It does not search equipment, recompute bonuses, request status or wait for an update. A present ghost is not the same as a destroyed Player.
- Luck reads the server’s status points, stored as UInt16, from status type >= 4. It is not a percentage, a random number or a guarantee of loot quality. The client does not derive a drop probability or add equipment bonuses during this read.
...
# Reads the current player’s status counter: luck points.
#
# Integer — luck points, 0..65535 in the client model. 0 can be a real value, unavailable data
# or a missing/destroyed Player. This is a quantity, not Boolean, ID or type: 1 means one unit,
# not success. It does not enumerate objects or return an array.
...
SUB Main()
    # LuckAtLeast(minimum) receives a numeric requirement; 1000 is an example, not a cap or
    # percentage. It rejects missing Player, reads luck once and returns Integer Boolean 1=TRUE or
    # 0=FALSE for value >= minimum. The luck counter itself is not Boolean. This only compares
    # cached points and does not predict a drop.
...
- LuckAtLeast(minimum) receives a numeric requirement; 1000 is an example, not a cap or percentage. It rejects missing Player, reads luck once and returns Integer Boolean 1=TRUE or 0=FALSE for value >= minimum. The luck counter itself is not Boolean. This only compares cached points and does not predict a drop.
