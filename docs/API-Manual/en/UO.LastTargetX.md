...
Reads the X coordinate saved when the last target was selected.
...
Integer — saved X coordinate, not a screen pixel. 0 before selection/after Clear or for an unknown object; zero is also a valid coordinate. Not Boolean. Land/static coordinates work even when LastTarget()=0.
...
Project source: `external/InjectionScript/src/InjectionScript/Runtime/InjectionApiUO.cs`; function `LastTargetX`.
...
# Reads the X coordinate saved when the last target was selected.
#
# Integer — saved X coordinate, not a screen pixel. 0 before selection/after Clear or for an
