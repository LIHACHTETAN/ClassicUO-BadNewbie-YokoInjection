...
Decimal — sin(radians). Input is radians, not degrees or a game direction. Finite result lies in [-1,1]; NaN or infinite input yields NaN. A numeric result, not an ID or a success flag; do not interpret 1/0 as success/failure here.
...
# Computes sine.
#
# Decimal — sin(radians). Input is radians, not degrees or a game direction. Finite result lies
...
SUB Main()
    # radians = 1.5707963267948966; expected result ~1 (~ means approximate). value stores the
...
- radians = 1.5707963267948966; expected result ~1 (~ means approximate). value stores the result and CStr only formats it for Print.
...
SUB Main()
    # degrees is an angle in degrees; multiplying by pi/180 gives radians before Sin.
    # SineDegrees(30) is approximately 0.5.
...
- degrees is an angle in degrees; multiplying by pi/180 gives radians before Sin. SineDegrees(30) is approximately 0.5.
