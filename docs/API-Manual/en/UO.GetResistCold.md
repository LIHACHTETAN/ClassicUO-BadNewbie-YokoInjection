...
Reads the current player’s resistance field: cold.
...
- The game-thread read takes Player.ColdResistance if the current Player exists and is not destroyed; otherwise 0. It does not search equipment, recompute bonuses, request status or wait for an update. A present ghost is not the same as a destroyed Player.
...
SUB Main()
    ...
    VAR before = UO.GetResistCold()
    WAIT(500)
    VAR after = UO.GetResistCold()
