...
Читает компоненты домов в указанной мировой клетке.
...
- `X` — Мировая координата X клетки текущей карты, не смещение внутри дома.
- `Y` — Мировая координата Y клетки. Возвращаются компоненты всех загруженных домов в этой клетке, на всех высотах Z.
...
SUB Main()
    # X/Y — текущая позиция персонажа.
    # Длина — число компонент, не их высота и не число домов.

    VAR parts = UO.GetMultiPartsAtPosition(UO.GetX(), UO.GetY())
