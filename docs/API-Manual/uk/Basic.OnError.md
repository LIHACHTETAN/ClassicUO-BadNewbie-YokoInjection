...
- Якщо помилка вийшла з Try після очищення та потрапила до зовнішнього On Error GoTo, Resume повторює весь Try із заголовка. Resume Next продовжує з першого рядка після End Try; On Error Resume Next діє так само. Уже виконані дії можуть повторитися: повернення всередину завершеного тіла немає.
