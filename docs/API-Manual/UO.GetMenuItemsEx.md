...
ClassicUO • Runtime API • `UO.GetMenuItemsEx.md`
...
- `UO.GetMenuItemsEx(Caption:String) -> Array`
  - **Return type:** `Array`
...
- `Caption` — Text/String value (String); pass literal text as a quoted BASIC string.

### Accepted values / constants

- `Caption` — Quoted BASIC string. Fixed keywords/enums, when present, are listed in Behavior/Notes.
...
```vb
SUB Main()
    VAR result = UO.GetMenuItemsEx(1)
...
SUB Main()
    VAR arg1 = 1 # Caption
    VAR result = UO.GetMenuItemsEx(arg1)
    UO.Print(CStr(result))
END SUB
```
