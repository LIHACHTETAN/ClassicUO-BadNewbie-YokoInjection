...
Integer — serial/ID，不是 type 或 Boolean。0 表示沒有物件目標；土地/靜態物件也為 serial 0，但可能保存了座標。保留全部 32 位元。請判斷 <> 0，而非 > 0 或 = TRUE。
...
# 讀取最後保存的物件目標 serial。
#
# Integer — serial/ID，不是 type 或 Boolean。0 表示沒有物件目標；土地/靜態物件也為 serial 0，但可能保存了座標。保留全部 32 位元。請判斷 <>
# 0，而非 > 0 或 = TRUE。
