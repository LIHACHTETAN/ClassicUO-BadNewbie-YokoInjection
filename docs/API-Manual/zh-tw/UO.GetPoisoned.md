...
- `value` — 所列呼叫形式中的選用物件：數字或十六進位字串 serial、self、lasttarget，或已登錄的 AddObject 名稱。不是 type。省略時讀取 self。某些形式會因未知文字而產生轉換錯誤；請先檢查名稱。

## 傳回值

Integer Boolean：1 = TRUE，0 = FALSE。可與數字或不加引號的邏輯常數比較。 mobile 具有 IsPoisoned 時回傳 1；否則或不存在時回傳 0。不回傳毒性強度或剩餘時間。
...
- Poisoned/Flying 依通訊協定版本判斷：7.0.0.0 之前，位元 0x04 表示中毒；從 7.0.0.0 起表示飛行，中毒狀態另行儲存。飛行不由 Z 推算。
...
mobile 具有 IsPoisoned 時回傳 1；否則或不存在時回傳 0。不回傳毒性強度或剩餘時間。
...
# 檢查 mobile 是否中毒。
#
# Integer Boolean：1 = TRUE，0 = FALSE。可與數字或不加引號的邏輯常數比較。 mobile 具有 IsPoisoned 時回傳 1；否則或不存在時回傳
# 0。不回傳毒性強度或剩餘時間。
#
