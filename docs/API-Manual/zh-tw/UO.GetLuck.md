...
Integer — 幸運點數，模型範圍 0..65535。0 可能是真實值、未知資料或 Player 不存在／已銷毀。這是數量，不是 Boolean、ID 或 type：1 表示一單位，不代表成功。不列舉物件，也不回傳陣列。

## 行為

- 本指令無參數。請使用上方精確語法。
- 在遊戲執行緒讀取 Player.Luck；目前 Player 不存在或已銷毀時回傳 0。不搜尋裝備、不重算加成、不要求狀態封包，也不等待更新。仍存在的鬼魂不等於已銷毀的 Player。
- Luck 讀取伺服器的幸運點數，以 UInt16 儲存，由 status type >= 4 提供。不是百分比、亂數或戰利品品質保證。讀取時不計算掉落機率，也不追加裝備加成。
...
# 讀取目前玩家的狀態計數值：幸運點數。
#
# Integer — 幸運點數，模型範圍 0..65535。0 可能是真實值、未知資料或 Player 不存在／已銷毀。這是數量，不是 Boolean、ID 或 type：1
...
SUB Main()
    # LuckAtLeast(minimum) 接受數值需求；1000 是範例，不是上限或百分比。拒絕不存在的 Player，讀取一次幸運值，再依 value >= minimum 回傳
    # Integer Boolean：1=TRUE 或 0=FALSE。幸運點數本身不是 Boolean。只比較快取，不預測掉落物。
...
- LuckAtLeast(minimum) 接受數值需求；1000 是範例，不是上限或百分比。拒絕不存在的 Player，讀取一次幸運值，再依 value >= minimum 回傳 Integer Boolean：1=TRUE 或 0=FALSE。幸運點數本身不是 Boolean。只比較快取，不預測掉落物。
