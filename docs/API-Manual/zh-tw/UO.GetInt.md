...
- `ObjID` — ObjID 僅用於顯示的一參數形式：角色 serial 整數、十進位／0x 十六進位字串、"self"、"lasttarget" 或 AddObject 名稱。它指定角色，不是 graphic/type。省略時讀取目前玩家。
...
- 支援 () 讀取 self，以及 (ObjID) 指定對象。沒有其他可選參數。
- Player 存在且未銷毀時讀取 Player.Intelligence，否則為 0。仍存在的死亡角色不是已銷毀物件。不會從 HP、法力或耐力推算此屬性。
...
- 以下名稱等價，可有或無 UO.，不區分大小寫： `Int Intelligence GetInt GetIntelligence`.
- 不帶 UO. 的 Int(value) 將 BASIC 數字向下取整；Str(value) 將值格式化為文字。它們不同於讀取屬性的 UO.Int()/UO.Str()。GetInt(ObjID) 不會進行數字取整。
- ExecuteStealthCompatibility.Arg 先解析物件名稱，再轉換數值。未知且非數字的文字會產生轉換錯誤，不會開啟目標游標。Decimal 轉為 Integer；Array/Unit 轉為 0。請使用有效 serial。
...
ObjID 僅用於顯示的一參數形式：角色 serial 整數、十進位／0x 十六進位字串、"self"、"lasttarget" 或 AddObject 名稱。它指定角色，不是 graphic/type。省略時讀取目前玩家。
...
專案原始碼: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 函式 `GetIntelligence`.

#### 4. Invoke

Player 存在且未銷毀時讀取 Player.Intelligence，否則為 0。仍存在的死亡角色不是已銷毀物件。不會從 HP、法力或耐力推算此屬性。 Invoke 在遊戲執行緒讀取，等待可隨腳本取消。不會請求 status、開啟 target、傳送封包、修改屬性或加入內建延遲。
...
CharacterStatus 處理適用的自身 status 封包時，將收到的 STR/DEX/INT 欄位寫入 Player.Intelligence。查詢只讀此快取，不等待新封包。
...
SUB Main()
    # id 來自 UO.Self()，不是 type。AddObject 因提供第二參數而直接將 ID 存為 statSubject，不開啟游標。兩次呼叫指定相同角色，但讀取並非原子快照。
