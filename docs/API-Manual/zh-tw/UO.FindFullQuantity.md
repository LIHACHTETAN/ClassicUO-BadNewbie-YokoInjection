...
Integer — 找到物品的 max(1, Amount) 加總，每個找到的角色另加 1。沒有結果為 0。兩疊各 50 會得到 100，FindCount() 則得到 2。總數是在搜尋時儲存的快照。
...
# 讀取上次搜尋時儲存的總數量。
#
# Integer — 找到物品的 max(1, Amount) 加總，每個找到的角色另加 1。沒有結果為 0。兩疊各 50 會得到 100，FindCount() 則得到
# 2。總數是在搜尋時儲存的快照。
