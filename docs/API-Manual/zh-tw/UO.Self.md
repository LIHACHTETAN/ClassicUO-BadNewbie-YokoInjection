...
Integer — serial/ID，不是 graphic/type、裝備層、物品數量或 Boolean。0 表示沒有目前物件。保留全部 32 位元；請用 <> 0，不要用 = TRUE 或 > 0。儲存的結果不會自動更新。 讀取 World.Player.Serial；Player 不存在或已銷毀時為 0。死亡不等於銷毀：仍存在的幽靈保留角色 ID。
...
讀取 World.Player.Serial；Player 不存在或已銷毀時為 0。死亡不等於銷毀：仍存在的幽靈保留角色 ID。 Invoke 在遊戲執行緒讀取；工作執行緒等待管理器處理請求。取消腳本會中斷等待。不增加額外延遲或網路查詢。
...
# 或 > 0。儲存的結果不會自動更新。 讀取 World.Player.Serial；Player 不存在或已銷毀時為 0。死亡不等於銷毀：仍存在的幽靈保留角色 ID。
