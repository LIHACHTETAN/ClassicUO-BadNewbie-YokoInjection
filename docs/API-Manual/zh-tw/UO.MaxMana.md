...
Integer — 欄位 ManaMax 的值，不是百分比或 Boolean。0 可能是真實數值，也可能表示缺少資料。不能以 0 的最大值作為除數。 其他 mobile 的數值可能未知。HP/HitsMax 可能是伺服器的相對刻度，而非精確點數。HP=0 不能證明死亡；請用 Dead/IsDead。
...
# 從本機模型讀取最大魔力。
#
# Integer — 欄位 ManaMax 的值，不是百分比或 Boolean。0 可能是真實數值，也可能表示缺少資料。不能以 0 的最大值作為除數。 其他 mobile
