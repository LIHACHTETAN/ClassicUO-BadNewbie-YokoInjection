...
Decimal — sin(radians). 輸入為弧度，不是角度或遊戲方向。有限結果在 [-1,1]；NaN/無限輸入得 NaN。 數值，不是 ID 或成功旗標。此處 1/0 不代表成功/失敗。
...
# 計算正弦。
#
# Decimal — sin(radians). 輸入為弧度，不是角度或遊戲方向。有限結果在 [-1,1]；NaN/無限輸入得 NaN。 數值，不是 ID 或成功旗標。此處 1/0
# 不代表成功/失敗。
...
SUB Main()
    # radians = 1.5707963267948966；預期 ~1（~ 表示近似）。value 保存結果，CStr 只為 Print 格式化。
...
SUB Main()
    # degrees 是角度，pi/180 轉弧度後呼叫 Sin。SineDegrees(30)≈0.5。
