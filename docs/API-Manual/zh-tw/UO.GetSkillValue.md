...
Decimal (Double) — 技能點數，步進為 0.1，例如 95.1 而非 951。欄位 BaseFixed 直接以 Double 除以 10。0 表示技能值為零，或角色/技能不存在。不是 Boolean。舊式 SkillVal/BaseVal 使用不同尺度，請勿混用。
...
# 讀取不含修正值的技能基礎值。
#
# Decimal (Double) — 技能點數，步進為 0.1，例如 95.1 而非 951。欄位 BaseFixed 直接以 Double 除以 10。0
