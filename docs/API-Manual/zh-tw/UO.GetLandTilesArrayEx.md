...
- `TileTypes` — 數值 graphic/type 的 Array。重複類型不會重複加入記錄。也接受單一數值作為一個類型。空 Array 表示所有類型；只包含一個 0 的陣列只搜尋 0。

## 傳回值

記錄 Array，每筆為 [graphic, X, Y, Z]，所有欄位都是 Integer。graphic 是地表類型，Z 是底部高度。沒有符合項目時傳回空 Array。 使用 GetArrayLength(result) 取得記錄數。索引從 0 開始。結果不是 Boolean、serial 或 Pascal record；沒有第七個輸出參數。
...
# 在矩形範圍內依 graphic/type 搜尋地表方塊。
#
# 記錄 Array，每筆為 [graphic, X, Y, Z]，所有欄位都是 Integer。graphic 是地表類型，Z 是底部高度。沒有符合項目時傳回空 Array。 使用
# GetArrayLength(result) 取得記錄數。索引從 0 開始。結果不是 Boolean、serial 或 Pascal record；沒有第七個輸出參數。
