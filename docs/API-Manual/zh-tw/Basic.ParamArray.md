...
```vb
# Sum() 取得空陣列並回傳 0。Sum(2,3,4) 取得三個值並回傳 9。For Each 逐一走訪。Main 回傳 Integer 9。
Option Explicit On
Function Sum(ParamArray values())
    Var total = 0
...
```vb
# values 包含 2 和 5。Forward 將陣列直接傳給 Sum，不增加包裝。Sum 相加並回傳 Integer 7。
Option Explicit On
Function Sum(ParamArray values())
    Var total = 0
    For Each value In values
...
```vb
# SetFirst(first,second) 修改新建陣列，first=2、second=3 不变。SetFirst(packed) 將共用 packed[0] 從 4 改為 9。Main 回傳 2*100+3*10+9，即 Integer 239。
Option Explicit On
