...
Integer — 寵物控制欄位上限，模型範圍 0..255。0 可能是真實值、未知資料或 Player 不存在／已銷毀。這是數量，不是 Boolean、ID 或 type：1 表示一單位，不代表成功。不列舉物件，也不回傳陣列。

## 行為

- 本指令無參數。請使用上方精確語法。
- 在遊戲執行緒讀取 Player.FollowersMax；目前 Player 不存在或已銷毀時回傳 0。不搜尋裝備、不重算加成、不要求狀態封包，也不等待更新。仍存在的鬼魂不等於已銷毀的 Player。
- PetsMax/FollowersMax 讀取 status type >= 3 的伺服器允許總欄位數，不是剩餘欄位。目前使用量可超過上限。顯示剩餘容量時算 maximum - current，再將負數改為 0。不釋放、馴服或召喚任何生物。
...
# 讀取目前玩家的狀態計數值：寵物控制欄位上限。
#
# Integer — 寵物控制欄位上限，模型範圍 0..255。0 可能是真實值、未知資料或 Player 不存在／已銷毀。這是數量，不是 Boolean、ID 或 type：1
