...
Integer — 保存的 X 座標，不是像素或 Boolean。選擇前、Clear 後或未知物件為 0；實際座標 0 也有效。LastTarget()=0 時仍可取得土地/靜態物件座標。
...
專案原始碼: `external/InjectionScript/src/InjectionScript/Runtime/InjectionApiUO.cs`; 函式 `LastTargetX`.
