...
Integer — 検索したアイテムの max(1, Amount) の合計に、見つかったキャラクター一体につき 1 を加えます。結果なしは 0。50 個のスタック二つなら 100、FindCount() は 2。検索時に保存された合計です。
...
# 直前の検索で保存された合計数量を読みます。
#
# Integer — 検索したアイテムの max(1, Amount) の合計に、見つかったキャラクター一体につき 1 を加えます。結果なしは 0。50 個のスタック二つなら
# 100、FindCount() は 2。検索時に保存された合計です。
