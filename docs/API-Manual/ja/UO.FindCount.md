...
検索結果の個数、または ID で指定したスタックの数量を読みます。
...
- `id` — FindCount でのみ省略可能。アイテムの serial：Integer、10 進／16 進文字列、lasttarget、lastobject、backpack、または AddObject 名。graphic/type でなく ID を渡します。未知の名前は 0。self はキャラクターなのでここでは 0。省略すると検索オブジェクト数を読みます。

## 戻り値

Integer — FindCount() は見つかったオブジェクト数です。一つのスタックは一つの item と数えます。FindCount(id) はそのアイテムの現在の Amount。未知／削除済み ID とキャラクターでは 0。非スタック品は通常 Amount=1。ID、type、Boolean ではありません。
...
# 検索結果の個数、または ID で指定したスタックの数量を読みます。
#
# Integer — FindCount() は見つかったオブジェクト数です。一つのスタックは一つの item と数えます。FindCount(id) はそのアイテムの現在の
# Amount。未知／削除済み ID とキャラクターでは 0。非スタック品は通常 Amount=1。ID、type、Boolean ではありません。
...
SUB Main()
    # lasttarget はゲーム内で選択済みのアイテムを指す必要があります。新しいターゲットカーソルは出ません。FindCount(id) は現在のスタックを読み、対象がなければ 0
    # を返し、検索結果を保持します。
...
- lasttarget はゲーム内で選択済みのアイテムを指す必要があります。新しいターゲットカーソルは出ません。FindCount(id) は現在のスタックを読み、対象がなければ 0 を返し、検索結果を保持します。
