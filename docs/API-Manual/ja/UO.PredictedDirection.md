...
プレイヤーが現在キューに入れている移動ステップの後に予測される方向を読みます。
...
Integer 方向 0..7。走行ビットを除去済み。0 は北、またはプレイヤー不在です。
...
- ワールド方向：0 北、1 北東、2 東、3 南東、4 南、5 南西、6 西、7 北西。画面は等角投影です。Direction.Mask が走行フラグを除去し、速度は返しません。
