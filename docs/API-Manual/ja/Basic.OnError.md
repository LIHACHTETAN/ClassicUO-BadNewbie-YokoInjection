...
- 後処理を終えて Try を離れたエラーが外側の On Error GoTo に届いた場合、Resume は Try 全体を先頭から再試行します。Resume Next と On Error Resume Next は End Try の次の文へ進みます。実行済みの処理が繰り返される場合があり、終了済みの本体の途中には戻りません。
