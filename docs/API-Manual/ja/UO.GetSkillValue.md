...
Decimal (Double) — 0.1 刻みのスキル点数。例は 951 ではなく 95.1。フィールド BaseFixed を直接 Double として 10 で割ります。0 はスキル値ゼロ、またはキャラクター/スキル不在です。Boolean ではありません。旧 SkillVal/BaseVal は別の尺度なので混用しないでください。
...
# 修正値を含まないスキルの基本値を読みます。
#
# Decimal (Double) — 0.1 刻みのスキル点数。例は 951 ではなく 95.1。フィールド BaseFixed を直接 Double として 10 で割ります。0
