...
現在のプレイヤーのステータス値を読み取ります：使用中のペット制御スロット。
...
Integer — 使用中のペット制御スロット、モデルの範囲は 0..255。0 は実値、未取得データ、Player 不在／破棄の可能性があります。数量であり Boolean、ID、type ではありません。1 は1単位で、成功の意味ではありません。物体を列挙せず、配列も返しません。

## 動作

- 引数はありません。表示された正確なシグネチャを使用します。
- ゲームスレッドで Player.Followers を読みます。現在の Player が存在しないか破棄済みなら 0。装備の検索、ボーナス計算、ステータス要求、更新待ちは行いません。存在する幽霊と破棄済み Player は別です。
- PetsCurrent/Followers は自身の status type >= 3 の使用中制御スロットを読みます。1匹が複数スロットを使う場合があり、召喚・調教された生物はサーバーの規則に従います。見える mobile 数、ペット ID 一覧、周囲の動物数ではありません。
...
# 現在のプレイヤーのステータス値を読み取ります：使用中のペット制御スロット。
#
# Integer — 使用中のペット制御スロット、モデルの範囲は 0..255。0 は実値、未取得データ、Player 不在／破棄の可能性があります。数量であり
