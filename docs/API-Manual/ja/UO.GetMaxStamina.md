...
Integer — フィールド StaminaMax の値であり、百分率や Boolean ではありません。0 は実際の値の場合も、データ不在の場合もあります。最大値 0 で割らないでください。 他の mobile の値は不明な場合があります。HP/HitsMax は正確な点数ではなくサーバーの相対的な尺度の場合があります。HP=0 は死亡の証明ではなく、Dead/IsDead を使用します。
...
# ローカルモデルから最大スタミナを読みます。
#
# Integer — フィールド StaminaMax の値であり、百分率や Boolean ではありません。0 は実際の値の場合も、データ不在の場合もあります。最大値 0
# で割らないでください。 他の mobile の値は不明な場合があります。HP/HitsMax は正確な点数ではなくサーバーの相対的な尺度の場合があります。HP=0
# は死亡の証明ではなく、Dead/IsDead を使用します。
...
SUB Main()
    # この名前には ID 形式がありません。引数なしの ReadValue は self を読み、二つの呼び出しを WAIT(1000)
    # で区切ります。二つのスナップショットの比較では途中の変化を見逃す場合があります。
...
- この名前には ID 形式がありません。引数なしの ReadValue は self を読み、二つの呼び出しを WAIT(1000) で区切ります。二つのスナップショットの比較では途中の変化を見逃す場合があります。
