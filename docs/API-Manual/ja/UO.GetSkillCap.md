...
Decimal (Double) — 0.1 刻みのスキル点数。例は 951 ではなく 95.1。フィールド CapFixed を直接 Double として 10 で割ります。0 はスキル値ゼロ、またはキャラクター/スキル不在です。Boolean ではありません。旧 SkillVal/BaseVal は別の尺度なので混用しないでください。
...
# サーバーから受け取ったスキル上限を読みます。
#
# Decimal (Double) — 0.1 刻みのスキル点数。例は 951 ではなく 95.1。フィールド CapFixed を直接 Double として 10 で割ります。0
