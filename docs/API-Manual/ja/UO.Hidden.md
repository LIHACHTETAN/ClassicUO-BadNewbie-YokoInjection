...
- `idText` — 表示された呼び出し形式で省略できる物体指定です。数値または16進文字列の serial、self、lasttarget、登録済み AddObject 名を指定します。type ではありません。省略時は self。形式によって未知の文字列は変換エラーになるため、先に名前を確認してください。
- `id` — 表示された呼び出し形式で省略できる物体指定です。数値または16進文字列の serial、self、lasttarget、登録済み AddObject 名を指定します。type ではありません。省略時は self。形式によって未知の文字列は変換エラーになるため、先に名前を確認してください。

## 戻り値

Integer Boolean：1 = TRUE、0 = FALSE。数値または引用符のない論理定数と比較できます。 mobile が IsHidden なら 1、それ以外または不在なら 0。サーバーフラグであり、壁越しの視線判定ではありません。
...
mobile が IsHidden なら 1、それ以外または不在なら 0。サーバーフラグであり、壁越しの視線判定ではありません。
...
# mobile が隠れているか調べます。
#
# Integer Boolean：1 = TRUE、0 = FALSE。数値または引用符のない論理定数と比較できます。 mobile が IsHidden なら 1、それ以外または不在なら
# 0。サーバーフラグであり、壁越しの視線判定ではありません。
#
