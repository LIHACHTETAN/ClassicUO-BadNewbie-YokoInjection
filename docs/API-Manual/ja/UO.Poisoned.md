...
Integer Boolean：1 = TRUE、0 = FALSE。数値または引用符のない論理定数と比較できます。 mobile が IsPoisoned なら 1、それ以外または不在なら 0。毒の強さや残り時間は返しません。
...
- Poisoned/Flying はプロトコルに従います。7.0.0.0 より前はビット 0x04 が毒を表し、7.0.0.0 以降は飛行を表します。毒は別に保持されます。Z から飛行を計算しません。
...
mobile が IsPoisoned なら 1、それ以外または不在なら 0。毒の強さや残り時間は返しません。
...
# mobile が毒状態か調べます。
#
# Integer Boolean：1 = TRUE、0 = FALSE。数値または引用符のない論理定数と比較できます。 mobile が IsPoisoned なら
# 1、それ以外または不在なら 0。毒の強さや残り時間は返しません。
#
