...
クライアントが最後に状態を受理した物体の serial を返します。
...
Integer — serial の32ビット値であり、type や Boolean ではありません。0 は未受理またはワールド消去後です。最上位ビットを保持するため負数も serial を表せます。> 0 や = TRUE ではなく <> 0 で確認します。
...
# クライアントが最後に状態を受理した物体の serial を返します。
#
# Integer — serial の32ビット値であり、type や Boolean ではありません。0 は未受理またはワールド消去後です。最上位ビットを保持するため負数も serial
# を表せます。> 0 や = TRUE ではなく <> 0 で確認します。
