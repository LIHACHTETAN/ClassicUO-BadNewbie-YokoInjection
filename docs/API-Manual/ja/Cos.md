...
Decimal — cos(radians). 入力はラジアンで、度やゲーム方向ではありません。有限結果は [-1,1]。NaN/無限大入力は NaN です。 数値であり ID や成功フラグではありません。1/0 を成功/失敗と解釈しないでください。
...
# 余弦を計算します。
#
# Decimal — cos(radians). 入力はラジアンで、度やゲーム方向ではありません。有限結果は [-1,1]。NaN/無限大入力は NaN です。 数値であり ID
# や成功フラグではありません。1/0 を成功/失敗と解釈しないでください。

SUB Main()
    # radians = 0、期待値 1（~ は近似）。value が結果を保存し、CStr は Print 用の表示に変換します。
...
SUB Main()
    # radians = 3.141592653589793、期待値 ~-1（~ は近似）。value が結果を保存し、CStr は Print 用の表示に変換します。
...
SUB Main()
    # degrees は度。pi/180 でラジアンにして Cos を呼びます。CosineDegrees(60)≈0.5。
