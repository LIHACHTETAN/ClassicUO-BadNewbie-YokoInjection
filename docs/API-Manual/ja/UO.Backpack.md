...
Integer — serial/ID です。graphic/type、レイヤー、個数、Boolean ではありません。0 は現在のオブジェクトなし。32 ビットを保持するため、= TRUE や > 0 ではなく <> 0 で確認します。保存した値は自動更新されません。 Player の Backpack レイヤー上の未破棄アイテムを探します。Player またはバッグが存在しない／破棄済みなら 0。コンテナ ID であり中身ではなく、新しいバッグも作成しません。
...
Player の Backpack レイヤー上の未破棄アイテムを探します。Player またはバッグが存在しない／破棄済みなら 0。コンテナ ID であり中身ではなく、新しいバッグも作成しません。 Invoke はゲームスレッドで読み取ります。ワーカースレッドはマネージャーによる処理を待ち、スクリプトのキャンセルで待機を中断します。追加の遅延や通信要求はありません。
...
Player の Backpack レイヤー上の未破棄アイテムを探します。Player またはバッグが存在しない／破棄済みなら 0。コンテナ ID であり中身ではなく、新しいバッグも作成しません。

Integer — serial/ID です。graphic/type、レイヤー、個数、Boolean ではありません。0 は現在のオブジェクトなし。32 ビットを保持するため、= TRUE や > 0 ではなく <> 0 で確認します。保存した値は自動更新されません。

プロジェクトのソース: `src/ClassicUO.Client/Game/GameObjects/Entity.cs`; 関数 `FindItemByLayer`.
...
# TRUE や > 0 ではなく <> 0 で確認します。保存した値は自動更新されません。 Player の Backpack レイヤー上の未破棄アイテムを探します。Player
# またはバッグが存在しない／破棄済みなら 0。コンテナ ID であり中身ではなく、新しいバッグも作成しません。
