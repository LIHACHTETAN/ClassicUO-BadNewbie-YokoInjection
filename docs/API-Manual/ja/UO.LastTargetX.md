...
Integer — 保存済み X 座標。画素や Boolean ではありません。選択前/Clear 後や不明な物体では0ですが、実際の座標0も有効です。LastTarget()=0 でも地面/静的物体の座標を取得できます。
...
プロジェクトのソース: `external/InjectionScript/src/InjectionScript/Runtime/InjectionApiUO.cs`; 関数 `LastTargetX`.
...
# 最後のターゲット選択時に保存した X を読みます。
#
# Integer — 保存済み X 座標。画素や Boolean ではありません。選択前/Clear 後や不明な物体では0ですが、実際の座標0も有効です。LastTarget()=0
# でも地面/静的物体の座標を取得できます。
