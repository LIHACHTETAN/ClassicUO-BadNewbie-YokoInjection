...
Integer — フィールド HitsMax の値であり、百分率や Boolean ではありません。0 は実際の値の場合も、データ不在の場合もあります。最大値 0 で割らないでください。 他の mobile の値は不明な場合があります。HP/HitsMax は正確な点数ではなくサーバーの相対的な尺度の場合があります。HP=0 は死亡の証明ではなく、Dead/IsDead を使用します。
...
# ローカルモデルから最大体力を読みます。
#
# Integer — フィールド HitsMax の値であり、百分率や Boolean ではありません。0 は実際の値の場合も、データ不在の場合もあります。最大値 0 で割らないでください。
...
SUB Main()
    ...
    VAR value = UO.MaxLife()
    UO.Print('MaxLife: ' + STR(value))
END SUB
```
