...
- `ObjID` — 省略可能な mobile の serial/ID。整数、16進文字列、self、lasttarget、ほかの標準オブジェクト別名、または AddObject の名前を指定します。graphic/type ではありません。省略時は self。解決できない別名は 0 になり、ターゲットカーソルは開きません。
