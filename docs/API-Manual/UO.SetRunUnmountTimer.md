...
ClassicUO • Runtime API • `UO.SetRunUnmountTimer.md`
...
- `UO.SetRunUnmountTimer(Value:Integer) -> Unit`
  - **Return type:** `Unit`
...
Sets the ClassicUO player movement completion delay used when **running on foot**. The value is propagated from Basic runtime state into `MovementSpeed.TimeToCompleteMovement`; it is no longer a state-only compatibility variable.
