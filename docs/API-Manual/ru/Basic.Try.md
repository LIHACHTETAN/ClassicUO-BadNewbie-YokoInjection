...
- Если ошибка вышла из Try после завершения очистки и попала во внешний обработчик On Error GoTo, Resume повторяет весь Try с заголовка. Resume Next продолжает с первой строки после End Try; On Error Resume Next действует так же. Уже выполненные действия при повторе могут повториться: вход обратно в завершённое тело не выполняется.
