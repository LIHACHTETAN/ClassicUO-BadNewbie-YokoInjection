...
### 2. Передача готового массива дальше

```vb
# values содержит 2 и 5. Forward получает готовый массив и передаёт его в Sum без дополнительной упаковки. Sum складывает два числа и возвращает Integer 7.
Option Explicit On
Function Sum(ParamArray values())
    Var total = 0
    For Each value In values
...
### 3. Скаляры и готовый массив

```vb
# SetFirst(first,second) меняет новый упакованный массив: first=2 и second=3 сохраняются. SetFirst(packed) меняет общий packed[0] с 4 на 9. Main возвращает 2*100+3*10+9, Integer 239.
Option Explicit On
