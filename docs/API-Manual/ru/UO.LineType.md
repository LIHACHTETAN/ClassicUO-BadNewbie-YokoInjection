...
Возвращает категорию текста выбранной записи журнала.
...
Integer — категория TextType клиента: 0 CLIENT, 1 SYSTEM, 2 OBJECT, 3 GUILD_ALLY. Без выбора 0. Это категория отображения, не serial, не graphic и не MessageType.
...
- Первичный справочник: https://stealth.od.ua/api/LineType/ .
...
# Возвращает категорию текста выбранной записи журнала.
#
# Integer — категория TextType клиента: 0 CLIENT, 1 SYSTEM, 2 OBJECT, 3 GUILD_ALLY. Без выбора
# 0. Это категория отображения, не serial, не graphic и не MessageType.
...
SUB Main()
    ...
    IF UO.InJournal('notice') > 0 THEN
        VAR value = UO.LineType()
        UO.Print('Text category: ' + STR(value))
    END IF
END SUB
```
