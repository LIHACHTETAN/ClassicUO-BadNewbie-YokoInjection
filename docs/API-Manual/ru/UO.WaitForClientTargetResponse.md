...
Ждёт завершения локального выбора ClientRequestObjectTarget/ClientRequestTileTarget.
...
- `MaxWaitTimeMS` — Максимальное время в миллисекундах. 0 и отрицательные значения дают одну проверку, без бесконечного ожидания.

## Возвращает

Integer 1 — ответ уже получен, в том числе отмена; 0 — истёк тайм-аут. При 1 всё равно проверяйте длину ClientTargetResponse. При 0 сам курсор не закрывается автоматически.
...
- Ожидание проходит в потоке скрипта с интервалом до 25 мс и прерывается остановкой скрипта.
- Без предыдущего ClientRequest* новый выбор не начинается.
...
# Ждёт завершения локального выбора ClientRequestObjectTarget/ClientRequestTileTarget.
#
# Integer 1 — ответ уже получен, в том числе отмена; 0 — истёк тайм-аут. При 1 всё равно
# проверяйте длину ClientTargetResponse. При 0 сам курсор не закрывается автоматически.
#
