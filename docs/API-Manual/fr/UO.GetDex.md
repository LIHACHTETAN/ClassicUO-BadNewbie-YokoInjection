...
Lit la Dextérité actuelle (Dexterity, DEX).
...
- Lit Player.Dexterity si Player existe et n’est pas détruit, sinon 0. Un personnage mort encore présent n’est pas un objet détruit. Aucune déduction à partir des HP, du mana ou de l’endurance.
...
- Noms équivalents avec ou sans UO., insensibles à la casse : `Dex Dexterity GetDex GetDexterity`.
...
Source du projet: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; fonction `GetDexterity`.

#### 4. Invoke

Lit Player.Dexterity si Player existe et n’est pas détruit, sinon 0. Un personnage mort encore présent n’est pas un objet détruit. Aucune déduction à partir des HP, du mana ou de l’endurance. Invoke lit sur le fil du jeu ; l’attente respecte l’annulation du script. Aucun paquet, demande de status, ciblage, modification ni délai intégré.
...
CharacterStatus affecte le champ STR/DEX/INT reçu à Player.Dexterity pour un paquet de status personnel applicable. La commande lit ce cache sans attendre un nouveau paquet.
