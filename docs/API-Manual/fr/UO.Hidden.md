...
- `idText` — Objet facultatif dans les formes affichées : serial numérique ou chaîne hexadécimale, self, lasttarget ou nom AddObject enregistré. Ce n’est pas un type. Sans argument, lit self. Un texte inconnu provoque une erreur de conversion dans certaines formes ; vérifiez le nom.
