...
Decimal — cos(radians). Radians, pas degrés ni direction du jeu. Résultat fini dans [-1,1] ; NaN/infini donnent NaN. Nombre, pas un ID ni un indicateur de réussite. 1/0 ne signifient pas réussite/échec.
...
# Calcule le cosinus.
#
# Decimal — cos(radians). Radians, pas degrés ni direction du jeu. Résultat fini dans [-1,1] ;
# NaN/infini donnent NaN. Nombre, pas un ID ni un indicateur de réussite. 1/0 ne signifient pas
# réussite/échec.

SUB Main()
    # radians = 0 ; résultat attendu 1 (~ : approximation). value conserve le résultat ; CStr le
    # formate pour Print.
...
- radians = 0 ; résultat attendu 1 (~ : approximation). value conserve le résultat ; CStr le formate pour Print.
...
SUB Main()
    # radians = 3.141592653589793 ; résultat attendu ~-1 (~ : approximation). value conserve le
...
- radians = 3.141592653589793 ; résultat attendu ~-1 (~ : approximation). value conserve le résultat ; CStr le formate pour Print.
...
SUB Main()
    # degrees est en degrés ; pi/180 convertit avant Cos. CosineDegrees(60)≈0.5.
