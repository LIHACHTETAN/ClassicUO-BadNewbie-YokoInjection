...
- `RadiobuttonID` — Integer — ID exact du type de contrôle concerné. Ce n’est ni un indice, ni un GumpID, ni le serial de la fenêtre. 0 est un ID ordinaire, jamais le premier contrôle par défaut.
- `Value` — Integer — 1 active, 0 désactive. Tout autre nombre non nul active aussi ; préférez 0/1. Sélectionner un bouton radio désélectionne les autres du même groupe.
