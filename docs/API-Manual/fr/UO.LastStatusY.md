...
Renvoie la coordonnée Y mémorisée du dernier objet de statut accepté.
...
Integer — coordonnée Y mémorisée, pas un pixel de fenêtre ni un booléen. 0 avant le premier statut ou après réinitialisation ; zéro est aussi une coordonnée valide. Vérifier UO.LastStatus() pour distinguer une absence.
...
Source du projet: `external/InjectionScript/src/InjectionScript/Runtime/InjectionApiUO.cs`; fonction `LastStatusY`.
...
# Renvoie la coordonnée Y mémorisée du dernier objet de statut accepté.
#
# Integer — coordonnée Y mémorisée, pas un pixel de fenêtre ni un booléen. 0 avant le premier
