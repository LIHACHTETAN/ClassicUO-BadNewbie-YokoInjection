...
Lit le compteur de status du joueur actuel : maximum d’emplacements de contrôle.
...
Integer — maximum d’emplacements de contrôle, 0..255 dans le modèle. 0 peut être réel, inconnu ou dû à Player absent/détruit. Quantité, pas Boolean, ID ou type : 1 signifie une unité, pas un succès. Aucun parcours d’objets ni tableau retourné.

## Comportement

- Aucun argument. Respecter les signatures affichées.
- Lit Player.FollowersMax sur le fil du jeu si Player existe sans être détruit ; sinon 0. Aucun parcours de l’équipement, calcul de bonus, requête status ou attente. Un fantôme présent n’est pas un Player détruit.
- PetsMax/FollowersMax lit le total autorisé par le serveur dès le status type 3, pas les emplacements libres. L’occupation peut dépasser la limite. Pour afficher la place libre, calculer maximum - current puis ramener les négatifs à 0. Aucune libération, invocation ou domestication.
...
# Lit le compteur de status du joueur actuel : maximum d’emplacements de contrôle.
#
# Integer — maximum d’emplacements de contrôle, 0..255 dans le modèle. 0 peut être réel, inconnu
