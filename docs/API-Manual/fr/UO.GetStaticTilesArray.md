...
Recherche les enregistrements Static/Multi/Item par graphic/type dans un rectangle.
...
- `TileType` — Un graphic/type de tuile, normalement 0..65535, pas un serial. 0 correspond exactement au graphic 0 ; -1 ne signifie pas tous les types. Aucun type correspondant : résultat vide.

## Retour

Array de lignes [graphic, X, Y, Z, hue], tous les champs Integer. Z est la hauteur de base, hue la couleur. Plusieurs lignes par case restent distinctes. Aucun résultat : Array vide. Le nombre de lignes est GetArrayLength(result). Les indices commencent à 0. Ce résultat n’est ni Boolean, ni serial, ni Pascal record ; aucun septième paramètre de sortie.
...
- La branche static actuelle inclut Static, Multi et Item, notamment les objets chargés au sol, mais exclut Land et Mobile. Elle dépasse donc les seules statiques des fichiers.
...
# Recherche les enregistrements Static/Multi/Item par graphic/type dans un rectangle.
#
# Array de lignes [graphic, X, Y, Z, hue], tous les champs Integer. Z est la hauteur de base,
# hue la couleur. Plusieurs lignes par case restent distinctes. Aucun résultat : Array vide. Le
# nombre de lignes est GetArrayLength(result). Les indices commencent à 0. Ce résultat n’est ni
# Boolean, ni serial, ni Pascal record ; aucun septième paramètre de sortie.
