...
s = replace_exact(s, unit_anchor, generator_block, 'Release 10 example regeneration gate')
save(rel, s)
...
s = replace_exact(
    ...
    '                if (procedure != null)\n'
    '                {\n'
    '                    // The fast path may observe a file created after an in-flight full scan\n'
    '                    // already enumerated the directory. Run it immediately, but force one\n'
    '                    // post-enumeration rescan so the authoritative catalogue catches up.\n'
...
for marker in ('_synchronousReloadInProgress', 'authoritative: true', 'TryLoadProcedureFromMatchingFileFast', 'post-enumeration rescan'):
...
if 'Regenerate Yoko engine examples from current runtime' not in build:
    raise RuntimeError('Release 10 example regeneration gate missing')
for relative_dir in ('Autoload', 'src/ClassicUO.Client/Autoload'):
    directory = root / relative_dir
...
print('v50 Release 10 source patch verification: PASS')
